(function(){
    ...
    if (! api) return;
    //Since we use custom_js in platformio.ini, this code only runs for the halmet build
    //No capability check needed - the build system handles environment-specific inclusion
    const tabName="halmet";
    const configName="exampleBDSel";
    const infoUrl='https://github.com/wellenvogel/esp32-nmea2000/tree/master/lib/halmettask';
    let boatItemName;
    let boatItemElement;
    api.registerListener((id, data) => {
        //data is capabilities - no check needed since custom_js is environment-specific
        ...
        // COmmented out to prevent spamming an non-existant endpoint
        // window.setInterval(() => {
