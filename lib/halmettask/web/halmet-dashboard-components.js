...
function MessageLogger() {
    ...
    const [parserStatus, setParserStatus] = useState({ state: 'idle', current: 0, total: 0, error: '' });
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!hdParser && typeof hdEnsureCanboat === 'function') {
                const canboat = await hdEnsureCanboat();
                if (!cancelled && canboat && canboat.FromPgn) {
                    hdParser = new canboat.FromPgn();
                    hdParser.on('error', () => {});
                    setParserReady(true);
                }
            }
    ...
    // Connect only after parser is ready to avoid TCP/WS pressure during chunk loading
    useEffect(() => {
    ...
    // Decoder progress indicator (fed from halmet-libraries.js)
    useEffect(() => {
        const evtName = 'halmet:canboat-status';
        const handler = (evt) => {
            if (evt && evt.detail) {
                setParserStatus(evt.detail);
            }
        };
        window.addEventListener(evtName, handler);
        if (window.FromPgn) {
            setParserStatus({ state: 'ready', current: 0, total: 0, error: '' });
        }
    ...
    const parserClass = parserStatus.state === 'ready' ? 'ready' : (parserStatus.state === 'error' ? 'error' : 'loading');
    let parserText = 'decoder idle';
    if (parserStatus.state === 'loading') {
        parserText = `decoder ${parserStatus.current || 0}/${parserStatus.total || 0}`;
    } else if (parserStatus.state === 'ready') {
        parserText = 'decoder ready';
    } else if (parserStatus.state === 'error') {
        parserText = 'decoder error';
    }
        ...
        <div class="halmet-dashboard">
            <div class="dashboard-header">
                ...
                <div class="dashboard-controls">
                    ...
                    <span class="parser-status ${parserClass}" title=${parserStatus.error || ''}>${parserText}</span>
