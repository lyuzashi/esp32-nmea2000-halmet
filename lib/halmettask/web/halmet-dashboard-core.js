...
async function hdLoadDependencies() {
    ...
    // Load lightweight UI runtime first; canboat is loaded on demand.
    const preactBundle = await hdLoadModule(HD_PREACT_BUNDLE_URL);
    ...
    console.log('Halmet: Base dependencies loaded');
}

/**
 * Lazy-load canboat only when parser/formatter features are needed.
 */
async function hdEnsureCanboat() {
    if (FromPgn && pgnToActisenseSerialFormat) {
        return {
            FromPgn,
            pgnToActisenseSerialFormat
        };
    }
    const canboatBundle = await hdLoadCanboat();
    ...
    console.log('Halmet: Canboat loaded', {
        FromPgn: !!FromPgn,
