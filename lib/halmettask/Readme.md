Halmet with NMEA2000 Gateway
============================


Notes:

* Investigate multiple task registration or separate sub-systems for the following features

ESP32-NMEA2000 uses Channels to share BoatData – this should be the source of truth 


Structure:
Halmet Task – board definition, board-specific inputs (analog - via I2C, digital pins)
NavlinkBlue Task – handles switching between BLE / Wifi and notifies all channel data with BLE messages
Victron Task – reads VE.Direct over UART from solar/shunt devices, translates and publishes NMEA2k messages
